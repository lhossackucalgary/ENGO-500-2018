...
------------ | ------------- | ------------- | ------------- | -------------
1 | Route crews to robots needing repairs or maintenance more optimally than appending each newly reported broken robot to the end of a crew's itinerary | Route-planning assistance software with ability to push notifications and itinerary to crew members' phones | Decrease in distance travelled for maintenance/ repair crews. ie. savings in fuel and travel time. | Measured by comparing solutions to sample problems consisting of randomly generated locations. The solution must outperform the trivial algorithm on over 95% of test cases. Performance is gauged by total distance travelled.
2 | Provide technicians with information to aid in diagnosing issues and performing repairs. Provide two AR overlays ontop of the robot through their mobile device (schematic diagram, historic data). | AR application able to provide a data overlay of historic and failure information for this robot and robot schematics | Technicians are able to view spatial data through AR to   |
3 | Consolidate failure & usage data to help understand why the robots need repairs, what fails most, etc. | Web-based IOC for analysts and dispatchers | Spatial and temporal patterns become more identifyable than tabular data | 
