...
The clients and users of this product will use the results of this project to maintain their robots more efficiently. By providing the clients with a dashboard showing when their robots will require maintenance or when they should perform maintenance, clients will reduce downtime. There will be three separate user bases in this project as described in the project description: the admin team, the IOC manager, and the maintenance crews. The admin team will handle the simulation 

Our project takes any robots that output sensor data and analyzes the data to determine when maintenance is required. Three separate groups of users will be involved in this project. The first group is the admin team who takes in the sensor data and analyzes it, performing routing algorithms as necessary based on the results of the analysis. The second group is the IOC Manager, who will be provided with a frontend display that visualizes the sensor data. Data that will be shown includes the current state of the robots, a prediction of when they will require maintenance, as well as a map of where the robots are located. When maintenance is needed, the manager will send out an alert to the nearest qualified worker to go and perform repairs. These maintenance workers are the third group of users, who will have a frontend display showing the most efficient route to the robot on a map. When they arrive at the location, the workers will then use an app involving Augmented Reality that will show them relevant data needed for maintenance or repairs.
